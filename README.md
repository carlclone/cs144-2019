...
5.没有人肉走完test case就开始写代码，结果是想的模型不对，不能满足后面的case，基本是推翻重写 （第一次是用segment，字符串的角度进行重组，但遇到了增量更新，未重组字节级别的覆盖，重组字节级别覆盖的case，最后把思考的模型改成字节流，代码也清晰许多）,这点让我想到业务开发里也是一样，没有往后思考可能的需求（更完善的test case ） ，导致代码只能东拼西凑满足功能，完全不堪入目。
